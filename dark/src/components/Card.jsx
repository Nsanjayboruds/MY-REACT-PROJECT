...
export default function Card() {
  ...
  const StarRating = ({ filled, className = '' }) => (
    <svg
      className={`w-4 h-4 ${filled ? 'text-yellow-300' : 'text-gray-200 dark:text-gray-600'} ${className}`}
      aria-hidden="true"
  ...
  return (
    <div className={`w-full bg-white border border-gray-200 rounded-lg shadow transition-colors duration-300 dark:bg-gray-800 dark:border-gray-700 ${themeMode === 'dark' ? 'dark' : 'light'}`}>
      <a href="/" className="block">
        <img 
          ...
          alt="Apple Watch product display" 
        />
      </a>
      <div className="px-5 pb-5">
        ...
        <div className="flex items-center mt-2.5 mb-5">
          {[...Array(4)].map((_, i) => (
            <StarRating key={`filled-${i}`} filled />
          ))}
